...
function AdminProfilePage() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (payload?.error) {
        showToast.error(payload.message || "Không thể tải thông tin hồ sơ");
        return;
      }
      const adminProfile = payload?.user ?? payload?.profile ?? payload;
      if (!adminProfile) {
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      const result = await userApi.updateAdminProfile({
        phone_number: editForm.phone_number || undefined,
        address: editForm.address || undefined,
      });
      const payload = result?.data ?? result;
      if (payload?.error) {
        showToast.error(payload.message || "Cập nhật thất bại");
        return;
      }
        ...
        {!isEditing ? (
          <div className="flex space-x-3">
            <button
              ...
              className="inline-flex items-center px-4 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 shadow-sm"
            >
